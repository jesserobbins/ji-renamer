...
/**
 * OOXML container types share the same ZIP + XML structure but differ in the
 * folder layout for their payload.  We track the extension families in sets so
 * dispatching later in the module remains a quick membership test instead of a
 * pile of string comparisons.
 */
...
/**
 * Legacy binary Office formats need to be converted to their OOXML cousins
 * before we can harvest text.  Each entry is a helpful warning that bubbles up
 * to the CLI when conversion is disabled.
 */
...
/**
 * OpenDocument bundles and Apple Keynote decks behave similarly to OOXML
 * packages but use different folder conventions.  Tracking them separately
 * keeps the routing logic self-documenting.
 */
...
// ZIP file signature constants used by parseZipEntries.  These magic numbers
// identify the end-of-central-directory record, the directory entries
// themselves, and the per-file headers in the archive respectively.
